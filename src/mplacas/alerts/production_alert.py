...
async def gather_production_alert_metrics(
    ...
    *,
    ...
    devices = await _gather_device_metrics(
        session,
        plant_id=plant_id,
...
def assess_production_alert(
    ...
    *,
    ...
    rendimento.
    """
    devices = _assess_devices(metrics.devices, device_normal_threshold)
...
def _assess_devices(
    ...
    for device in devices:
        ...
        assessed.append(
            DeviceYieldAssessment(device=device, relative_to_own_median=relative, dropped=dropped)
        )
...
async def _gather_device_metrics(
    session: AsyncSession,
    *,
    plant_id: uuid.UUID,
    target_date: date,
