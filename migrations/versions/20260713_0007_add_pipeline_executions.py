...
_execution_status = PgEnum(
    "RUNNING", "SUCCEEDED", "FAILED", name="pipelineexecutionstatus", create_type=False
)


def upgrade() -> None:
    op.execute(sa.text(
        "CREATE TYPE pipelineexecutionstatus AS ENUM ('RUNNING', 'SUCCEEDED', 'FAILED')"
    ))
    op.create_table(
        "pipeline_executions",
...
def downgrade() -> None:
    ...
    op.execute(sa.text("DROP TYPE IF EXISTS pipelineexecutionstatus"))
