...
> **Decisão tomada em 2026-08-27, depois deste estudo:** o dono declarou não ter interesse em pagar,
> por ser projeto de **uso pessoal**. Isso passou a ser restrição de projeto, registrada em
> `docs/POLITICA_CUSTO_ZERO.md` e travada por `tests/test_zero_cost_contract.py`. O estudo fica como
> memória de cálculo — a decisão não está mais em aberto.
...
## Se a franquia gratuita apertar

Como pagar saiu de cena, estes sinais deixam de ser "hora de assinar" e passam a ser **hora de
investigar**. Nas duas vezes em que a franquia estourou, a causa foi defeito, não crescimento
legítimo — o roteiro de resposta está em `docs/POLITICA_CUSTO_ZERO.md`.
...
3. **A aplicação ganhar usuários reais**, em que hibernar e ficar indisponível deixa de ser
   aceitável. Hoje isso não se aplica: é uso pessoal, e indisponibilidade é aceitável — cobrança não.
...
O item 4 é o único que reabriria a discussão de pagar, e mesmo assim exigiria decisão explícita do
dono em ADR novo — hoje pagar compra risco de fatura sem comprar capacidade de que se precise.
