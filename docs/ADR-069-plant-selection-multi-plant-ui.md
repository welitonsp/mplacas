...
**E9 — fan-out do agendador (`cloud_jobs.py`).** *(Era a "Etapa F".)* `run_daily_pipeline` e `run_operational_watchdog` deixam de resolver uma única usina por `MPLACAS_CLOUD_JOB_PLANT_NAME` e passam a iterar as usinas da organização, com a expectativa resolvida por usina (E6). **Sem esta sub-etapa o fan-out HTTP não faz a segunda usina ser coletada de madrugada** — ver E.11.7. Depende de E6. *Verificável*: job executado numa org com 2 usinas grava execução de pipeline para as duas; falha numa usina não impede a outra e o job termina com código de saída não-zero. **Reviewer.** **Entrada em escopo confirmada pelo usuário em 2026-08-06 — E.10 item 8.**

****Correção urgente de 2026-08-06 (achada em diagnóstico de produção, antes do job agendado real ser atualizado): deadlock circular corrigido.** `run_daily_pipeline` pulava a usina inteira quando `resolve_expected_daily_production` devolvia `None` (sem baseline sazonal). Como o baseline só se forma a partir do acúmulo de resultados de performance calculados dentro da própria execução do pipeline, uma usina sem baseline nunca conseguia rodar o passo de performance — travando para sempre. Corrigido: os passos de clima/POA/performance/baseline/perdas rodam incondicionalmente; apenas a etapa de despacho de alertas é suprimida (`outcome="skipped"`) quando não há expectativa derivável. **Acompanhamento registrado, não bloqueante**: `src/mplacas/orchestration/router.py` (fan-out HTTP administrativo da Etapa E7, modo implícito) tem o mesmo padrão de "pular usina sem expectativa", mas não é o único caminho de formação de baseline (o job noturno corrigido já resolve isso sozinho) — considerar aplicar a mesma correção ali por consistência numa iteração futura.

Exceção formalizada em 2026-08-06 (revisão do reviewer, implementação): `run_collection` fica FORA do fan-out desta sub-etapa, deliberadamente.** O NEPViewer é uma conta única/global por instalação (`MPLACAS_NEP_ACCOUNT`/`MPLACAS_NEP_PASSWORD`, configurações singulares, sem noção de múltiplas contas por usina), e `SolarDevice` (`providers/base.py`) não carrega nenhum campo que identifique a usina física de origem — `list_devices()` devolve uma lista plana sem distinção por usina. Fazer fan-out aqui não coletaria N streams de telemetria independentes: escreveria os dados da MESMA conta em TODAS as usinas da organização, corrompendo dados de produção. `run_collection` continua resolvendo uma única usina via `MPLACAS_CLOUD_JOB_PLANT_NAME`, documentado em docstring no código (`cloud_jobs.py`). Corrigir isso exigiria uma mudança de arquitetura fora do escopo deste ADR (ex: um campo de identificação de usina por dispositivo/conta na integração com o provider), registrada aqui como trabalho futuro, não como pendência desta etapa.
