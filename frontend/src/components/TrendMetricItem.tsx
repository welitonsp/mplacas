    ...
    <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 py-2.5">
      ...
      <div className="flex flex-col items-end gap-1">
        <span className={`inline-flex items-center gap-1.5 text-sm font-semibold ${SEVERITY_TEXT[severity]}`}>
          {/* O símbolo continua presente como reforço visual rápido, mas nunca é o
              único canal de direção: o texto (DIRECTION_TEXT) e a barra do
              Sparkline abaixo também codificam o sentido do delta. */}
        ...
        {percent != null && (
          <Sparkline percentDelta={percent} label={label} tone={severity} className="w-24" />
        )}
      </div>
    </div>
  )
