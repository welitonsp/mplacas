// Parser de "produção esperada" a partir de `GET /photovoltaic/summary`
// (ver ADR-065 e ADR-068). Arquivo separado de `contracts.ts` porque esse
// endpoint pertence a um domínio diferente (modelagem técnica fotovoltaica) e
// carrega um vocabulário próprio de motivo de indisponibilidade que não se
// mistura com os contratos de `/energy/*` já existentes. A produção esperada
// em si é calculada inteiramente no backend (ADR-068) — este arquivo só
// estrutura os campos já prontos que o servidor devolve, nunca compõe
// grandezas físicas.
import { toNumber } from '../format'

// Os motivos derivados pelo backend quando ainda não existe um baseline
// sazonal defensável (ADR-065, seção 6), mais `INCOMPLETE_EXPECTATION_INPUTS`
// (ADR-068, seção 2): existe baseline defensável, mas alguma das entradas da
// produção esperada (`dc_capacity_kwp`, `clear_sky_poa_p90_kwh_m2`,
// `baseline_median_performance_ratio`) veio nula ou não positiva. Esse quarto
// código só é emitido em `expected_daily_production_unavailable_reason`, nunca
// em `baseline_unavailable_reason`, mas o tipo é compartilhado entre os dois
// campos porque o segundo é um superconjunto do primeiro.
// `performance_unavailable_reason` e `losses_unavailable_reason` usam outro
// vocabulário (ex: `NO_PERFORMANCE_RESULTS`, `NO_LOSS_ASSESSMENTS`) que esta
// tela não consome — não validamos esses campos.
export type BaselineUnavailableReason =
  | 'NO_PERFORMANCE_HISTORY'
  | 'REFERENCE_YEAR_INCOMPLETE'
  | 'INSUFFICIENT_SEASONAL_SAMPLES'
  | 'INCOMPLETE_EXPECTATION_INPUTS'
...
// Produção diária esperada (kWh) para a usina — calculada e quantizada
// inteiramente no backend (`photovoltaic/expected_production.py`, ADR-068) a
// partir do baseline sazonal e da capacidade instalada. O cliente só lê os
// quatro campos aditivos de `GET /photovoltaic/summary`
// (`expected_daily_production_kwh/_model_version/_nature/_unavailable_reason`)
// e os expõe já estruturados aqui — nenhuma grandeza física é multiplicada
// neste arquivo. `modelVersion`/`nature` viajam para permitir, no futuro, um
// rótulo honesto sobre a natureza do número (ver ADR-068, seção 1).
export type ExpectedDailyProduction =
  | { available: true; kwh: number; modelVersion: string; nature: string }
...
export interface PhotovoltaicSummaryResponse {
  ...
  expectedProduction: ExpectedDailyProduction
}
...
// Lê os quatro campos aditivos de produção esperada (ADR-068, seção 3) e os
// estrutura em `ExpectedDailyProduction`. Nenhum cálculo acontece aqui: o
// backend já devolve `expected_daily_production_kwh` pronto, quantizado em
// `0.001` kWh — este parser só decide, a partir de valores nulos ou de
// `expected_daily_production_unavailable_reason`, qual das duas variantes do
// tipo devolver.
function parseExpectedDailyProduction(
  payload: Record<string, unknown>,
  referenceCompleteOn: string | null
): ExpectedDailyProduction {
  const reason = parseBaselineUnavailableReason(payload.expected_daily_production_unavailable_reason)
  const kwh = toNumber(optionalString(payload, 'expected_daily_production_kwh'))
  const modelVersion = optionalString(payload, 'expected_daily_production_model_version')
  const nature = optionalString(payload, 'expected_daily_production_nature')

  if (reason !== null || kwh === null || kwh <= 0 || modelVersion === null || nature === null) {
    return {
      available: false,
      reason: reason ?? 'NO_PERFORMANCE_HISTORY',
      referenceCompleteOn,
    }
  }

  return { available: true, kwh, modelVersion, nature }
}
...
export function parsePhotovoltaicSummary(payload: unknown): PhotovoltaicSummaryResponse {
  ...
  const referenceCompleteOn = nullableString(payload, 'reference_complete_on')
  return {
    ...
    expectedProduction: parseExpectedDailyProduction(payload, referenceCompleteOn),
  }
}
...
export function baselineUnavailableMessage(
  ...
  switch (reason) {
    ...
    case 'INCOMPLETE_EXPECTATION_INPUTS':
      return 'Produção esperada ainda não disponível — dados técnicos desta usina estão incompletos.'
  }
}
